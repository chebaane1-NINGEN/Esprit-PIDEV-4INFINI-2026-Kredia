...
interface TimelineEvent {
  id: string;
  type: 'CREATION' | 'ROLE_CHANGE' | 'STATUS_CHANGE' | 'LOGIN' | 'LOGOUT' | 'PASSWORD_CHANGE' | 'SECURITY_EVENT' | 'PROFILE_UPDATE' | 'ASSIGNMENT_CHANGE' | 'BULK_ACTION';
  title: string;
  description: string;
  timestamp: string;
  actor?: string;
...
const UserDetail: React.FC = () => {
  ...
  const fetchUserTimeline = async () => {
    try {
      ...
      const response = await userApi.getAdminAudit(Number(id));
      
      const formattedTimeline: TimelineEvent[] = response.content.map((log: any) => ({
        id: log.id.toString(),
        type: log.actionType,
        title: log.actionType.replace('_', ' '),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setUser({ ...user, ...editForm } as UserResponseDTO);
      setEditing(false);
  ...
  const handleStatusChange = async (newStatus: UserStatus) => {
    ...
    try {
      if (newStatus === UserStatus.BLOCKED) await userApi.block(Number(id));
      else if (newStatus === UserStatus.SUSPENDED) await userApi.suspend(Number(id));
      else if (newStatus === UserStatus.ACTIVE) await userApi.activate(Number(id));
  ...
  const getTimelineIcon = (type: string): any => {
    if (type.includes('CREATE')) return <UserCheck className="w-4 h-4 text-green-500" />;
    if (type.includes('ROLE')) return <Shield className="w-4 h-4 text-blue-500" />;
    if (type.includes('STATUS')) return <Activity className="w-4 h-4 text-yellow-500" />;
